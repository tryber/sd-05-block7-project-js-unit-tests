...
const createMenu = (obj) => {
  const meuRestaurante = {
    ...
    consumption: [],
    order: function (str) {
      meuRestaurante.consumption.push(str);
    },
    pay: function () {
      let conta = 0;
      meuRestaurante.consumption.forEach(item => {
        if(meuRestaurante.fetchMenu.food[item]) {
          conta += meuRestaurante.fetchMenu.food[item];
        } else if (meuRestaurante.fetchMenu.drinks[item]) {
          conta += meuRestaurante.fetchMenu.drinks[item];
        }
      })
      return conta;
    }
