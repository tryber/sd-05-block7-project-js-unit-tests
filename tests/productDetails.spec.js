...
describe('#productDetails', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    const aux1 = productDetails('Alcool gel', 'Máscara')[0];
    const aux2 = productDetails('Alcool gel', 'Máscara')[1];
    let output = null;
    if (aux1 === aux2) {
      output = true;
    } else {
    ...
    //ou
    assert.notStrictEqual(productDetails('Alcool gel', 'Máscara')[0].details.productId.lastIndexOf('123'), -1);
    assert.notStrictEqual(productDetails('Alcool gel', 'Máscara')[1].details.productId.lastIndexOf('123'), -1);
  });
});
