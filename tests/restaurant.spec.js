...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    const t1 = createMenu({});
    assert.deepStrictEqual(t1.fetchMenu, {});
    ...
    const t2 = createMenu({ food: {}, drink: {} }).fetchMenu;
    assert.deepStrictEqual(Object.keys(t2), ['food', 'drink']);    

    // --------------------------------------------------------------------------------------
    // TESTE 3: Verifique que o menu passado pra função createMenu é identico ao menu recuperado pela função 'objetoRetornado.fetchMenu'
    const t3 = createMenu({ food: {}, drink: {} }).fetchMenu;
    assert.deepStrictEqual(t3, { food: {}, drink: {} });
    
    // --------------------------------------------------------------------------------------
    // TESTE 4: Verifique que 'objetoRetornado.consumption', após a criação do menu, retorna um array vazio.
    const t4 = createMenu({ food: {}, drink: {} }).consumption;
    ...
    const t5 = createMenu({ food: {}, drink: {} }).order('coxinha');
    assert.deepStrictEqual(t5, ['coxinha']);

    // --------------------------------------------------------------------------------------
    // TESTE 6: Verifique que as três orders seguintes, de bebidas e comidas mescladas, somam três itens no array `objetoRetornado.consumption` conforme os itens pedidos.
    const t6 = createMenu({ food: {}, drink: {} });
    t6.order('coxinha');
    t6.order('agua');
    t6.order('sopa');
    t6.order('sashimi');
    assert.deepStrictEqual(t6.consumption, ['coxinha', 'agua', 'sopa', 'sashimi']);

    // --------------------------------------------------------------------------------------
    // TESTE 7: Verifique que a função `order` aceita que pedidos repetidos sejam acrescidos a consumption.
    const t7 = createMenu({ food: {coxinha: 3.90}, drink: {agua: 3.90} });
    t7.order('coxinha');
    t7.order('agua');
    t7.order('coxinha');  
    assert.deepStrictEqual(t7.consumption, ['coxinha', 'agua', 'coxinha']);

    // --------------------------------------------------------------------------------------
    // TESTE 8: Verifique que, ao chamar `objetoRetornado.pay()`, retorna-se a soma dos preços de tudo que foi pedido, conforme registrado em `objetoRetornado.consumption`
    assert.deepEqual(t7.pay(), (3.90 + 3.90).toFixed(2));
  });
});
