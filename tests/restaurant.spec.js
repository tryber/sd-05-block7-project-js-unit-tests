...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    const retorno = createMenu(input);
    const {fetchMenu} = retorno;
    assert.deepStrictEqual(fetchMenu, input);
    // --------------------------------------------------------------------------------------
    ...
    const retorno2 = createMenu({ food: {}, drink: {} }).fetchMenu;
    const chaves = Object.keys(retorno2);
    const esperado = ['food', 'drink'];
    assert.deepStrictEqual(chaves, esperado);
    ...
    assert.deepStrictEqual(retorno.fetchMenu, input);
    // const objetoRetornado = createMenu(objetoQualquer);
    ...
    assert.deepStrictEqual(retorno.consumption, [])
    // const objetoRetornado = createMenu(objetoQualquer);
    ...
    const objetoRetornado = createMenu(input);
    ...
    const retorno3 = createMenu(input);
    retorno3.order('coxinha');
    retorno3.order('agua');
    retorno3.order('coxinha');
    assert.deepStrictEqual(retorno3.consumption, ['coxinha', 'agua', 'coxinha']);
    // ```
    // Agora faça o TESTE 8 deste arquivo.
    // --------------------------------------------------------------------------------------
    // TESTE 8: Verifique que, ao chamar `objetoRetornado.pay()`, retorna-se a soma dos preços de tudo que foi pedido, conforme registrado em `objetoRetornado.consumption`
    assert.equal(retorno3.pay(), 11.70)
    // objetoRetornado.order('coxinha');
    // objetoRetornado.order('agua');
    // objetoRetornado.order('coxinha');
